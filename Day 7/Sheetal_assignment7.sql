...
SELECT 
    ...
    freight - LAG(freight) OVER(PARTITION BY customer_id ORDER BY order_date) AS freight_diff_from_previous,
    LEAD(freight) OVER(PARTITION BY customer_id ORDER BY order_date) - freight AS freight_diff_to_next
FROM 
    orders
ORDER BY 
...
WHEN unit_price < 20 THEN 'Low Price' WHEN unit_price < 50 THEN 'Medium Price' ELSE 'High Price'
...
WITH categorized_products AS (
    SELECT 
        ...
        price_in_usd,
        CASE 
            WHEN price_in_usd < 20 THEN 'Low Price'
            WHEN price_in_usd < 50 THEN 'Medium Price'
            ELSE 'High Price'
...
SELECT 
    ...
    ROUND(AVG(price_in_usd)::numeric, 2) AS avg_price
FROM 
